...
export class ReviewComponent implements OnInit {
  ...
  @ViewChild ('paypalContainer') paypalRef: ElementRef;
  public payPalConfig = null;
    ...
    this.shopService.getConfig().subscribe(config => {
       ...
       this.shopService.getCustomerDetails().subscribe(details => {
            ...
            this.basketService.getBasket().subscribe(basket => {
              ...
              // load paypal library
              get(`https://www.paypal.com/sdk/js?client-id=${this.paypalIdLive}&currency=${this.config.preferences.currency.chosen.toUpperCase()}`,
        ...
        return actions.order.create({
          ...
          purchase_units: [{
            amount: {
              ...
              value: this.basket.Costs[this.config.preferences.countryCode][this.config.preferences.deliveryMethod].
                payBeforeDelivery.total[this.config.preferences.currency.chosen.toLowerCase()].toString(),
              breakdown: {
                  item_total: {
                      currency_code: this.config.preferences.currency.chosen,
                      value: this.basket.Costs[this.config.preferences.countryCode][this.config.preferences.deliveryMethod].
                        payBeforeDelivery.subTotal[this.config.preferences.currency.chosen.toLowerCase()].toString()
                  },
                  discount: {
                    currency_code: this.config.preferences.currency.chosen,
                    value: this.basket.Costs[this.config.preferences.countryCode][this.config.preferences.deliveryMethod].
                      payBeforeDelivery.discount[this.config.preferences.currency.chosen.toLowerCase()].toString()
                  },
                  shipping: {
                    currency_code: this.config.preferences.currency.chosen,
                    value: this.basket.Costs[this.config.preferences.countryCode][this.config.preferences.deliveryMethod].
                      payBeforeDelivery.delivery[this.config.preferences.currency.chosen.toLowerCase()].toString()
                  }
              }
            },
            items: this.getPaypalBasketItems(this.basket.Items, this.config.preferences.currency.chosen, 'en'),
            description: 'Dome Kutya Order',
            custom_id: this.basket.BaskedId,
  ...
  getPaypalBasketItems(items: any, currency: string, lang: string): any{
    ...
    for (let i = 0; i < items.length; i++){
      ...
      paypalItems.push({name: itemName, sku: wholeId, unit_amount: price, 
          quantity: items[i].Quantity.toString(), currency, category: 'PHYSICAL_GOODS'});
    }
